...
-- Importing Data through COPY Command

COPY branch(branch_id, manager_id, branch_address, contact_no)
FROM 'C:\Users\Public\Documents\branch.csv'
DELIMITER ','
CSV HEADER;

ALTER TABLE branch
ALTER contact_no TYPE VARCHAR(30);

SELECT * FROM branch;

COPY employees(emp_id, emp_name, position, salary, branch_id)
FROM 'C:\Users\Public\Documents\employees.csv'
DELIMITER ','
CSV HEADER;

ALTER TABLE employees
ALTER salary TYPE DECIMAL(10,2);

SELECT * FROM employees;

COPY members(member_id, member_name, member_address, reg_date)
FROM 'C:\Users\Public\Documents\members.csv'
DELIMITER ','
CSV HEADER;
...
FROM 'C:\Users\Public\Documents\return_status.csv'
DELIMITER ','
CSV HEADER;

SELECT * FROM return_status;

/* NOTE :  I have imported the data for the tables books and issued_status
via Import/Export Data Feature of PostgreSQl.*/

SELECT * FROM issued_status;
SELECT * FROM books;

-- Creating relationships Among the Tables through Foreign Key:
...
CALL update_book_status('RS165','IS136');
...
CREATE TABLE branch_ AS (
	SELECT e.branch_id, ist.issued_emp_id
	FROM employees e
	JOIN issued_status ist
